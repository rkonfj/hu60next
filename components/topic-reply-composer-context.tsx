...
export function TopicReplyComposerProvider({
  ...
  const topicRef = useRef({ topicId, currentPage, floorReverse });

  floorUbbRef.current = floorUbbById;
  topicRef.current = { topicId, currentPage, floorReverse };
  ...
  useEffect(() => {
    async function handleContentAtClick(event: MouseEvent) {
      ...
      if (replyTrigger) {
        ...
        const { topicId: activeTopicId, currentPage: page, floorReverse: reverse } =
          topicRef.current;
        let quotedContent =
        ...
        if (!quotedContent && Number.isInteger(contentId) && contentId > 0) {
          quotedContent = await loadFloorUbb(
            activeTopicId,
            contentId,
            page,
            reverse
