...
import { sanitizeUserStyle } from "./user-css";
...
describe("sanitizeUserStyle border limits", () => {
  ...
  it("rejects oversized border-width declarations", () => {
    ...
    assert.equal(sanitizeUserStyle("border-width: 32rem"), "");
    assert.equal(sanitizeUserStyle("border: 32em solid red"), "");
  });

  it("allows border-width up to 32px", () => {
    assert.equal(sanitizeUserStyle("border-width: 32px"), "border-width: 32px");
    assert.equal(sanitizeUserStyle("border-width: 0"), "border-width: 0");
    assert.equal(
