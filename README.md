...
上游无需支持或解析这个标签。发帖接口直接从正文读取标题和选项；主题
发布并取得 ID 后，投票数据会写入本机 `data/topic/{topicId}.json` 的
`votes` 字段。帖子正文取回本站后，整段 UBB 会替换为当前主题的投票组件。
如果主题文件或 `votes` 字段不存在，主题页会从正文 UBB 惰性重建投票标题
和选项，并以 0 票初始化；已有投票文件不会被正文覆盖。UBB 本身不包含
历史票数和投票人，因此删除数据文件后无法恢复已经产生的投票结果。
